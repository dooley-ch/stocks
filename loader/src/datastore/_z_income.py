...
def zIncomeInsuranceQuarter_get_by_ticker(ticker: str, db_conn: pymysql.Connection) -> zIncomeInsuranceQuarterRecord | None:
    """
    ...
    with db_conn.cursor() as cursor:
        cursor.execute("""SELECT id, ticker, simfin_id, currency, fiscal_year, fiscal_period, report_date, publish_date, 
                            ...
                            FROM z_income_insurance_quarter WHERE (ticker = %s);""", (ticker,))
        row = cursor.fetchone()
        if row:
...
def zIncomeGeneralQuarter_get_by_ticker(ticker: str, db_conn: pymysql.Connection) -> zIncomeGeneralQuarterRecord | None:
    """
    This function returns all record from the table
    :param ticker: the ticker to filter by
    :param db_conn: the database_old connection to use
    ...
    with db_conn.cursor() as cursor:
        ...
        publish_date, 
                            restated_date, shares_basic, shares_diluted, revenue, cost_of_revenue, gross_profit, 
                            operating_expenses, selling_general_administrative, research_development, 
                            depreciation_amortization, operating_income, non_operating_income, interest_expense_net, 
                            pretax_income_loss_adj, abnormal_gains_lLosses, pretax_income_loss, 
                            income_tax_expense_benefit_net, income_loss_from_continuing_operations, 
                            net_extraordinary_gains_losses, net_income, net_income_common
                            FROM z_income_general_quarter WHERE (ticker = %s);""", (ticker,))
        row = cursor.fetchone()
        if row:
...
def zIncomeBankQuarter_get_by_ticker(ticker: str, db_conn: pymysql.Connection) -> zIncomeBankQuarterRecord | None:
    """
    This function returns all record from the table
    :param ticker: the ticker to filter by
    :param db_conn: the database_old connection to use
    ...
    with db_conn.cursor() as cursor:
        cursor.execute("""SELECT id, ticker, simfin_id, currency, fiscal_year, fiscal_period, report_date, publish_date, 
                            ...
                            income_loss_from_continuing_operations, net_extraordinary_gains_losses, net_income, 
                            net_income_Common
                            FROM z_income_bank_quarter WHERE (ticker = %s);""", (ticker,))
        row = cursor.fetchone()
        if row:
